...
class Exame
{
    ...
    function get()
    {
        ...
        $stm = $this->conexao->conectar()->prepare("select * from exame_periodico where ativo = true");
        $stm->execute();
    ...
    {
        $this->conexao->conectar();
        $stm = $this->conexao->conectar()->prepare("select id_exame,exame,laboratorio,data_exame,resultado from relatorio_funcionario r LEFT JOIN exame_periodico exa on r.fk_exame = exa.id_exame where r.fk_funcionario = :id");
        $stm->bindParam("id", $id);
        $stm->execute();
        $dados = $stm->fetchAll(PDO::FETCH_OBJ);
        return $dados;
